...
**Source Files**: Materials\UVM Theory.pdf, Materials\UVM_module5_HVM.pdf

**Total Pages/Slides**: 29

**Extraction Time**: 2026-05-15T03:22:56.631666
...
- **Main**
  - Difference between object and component macros:
  - 2. Connect Phase: Connect TLM ports/exports.
  - synchronized execution. But Objects like:
  - 4.1  What Is UVM?
  - 4.2  Polymorphism
  - 4.3  UVM Hierarchy
  - 4.3.1  UVM Testbench
...
# Source: UVM Theory.pdf


## Page 1: Difference between object and component macros:


## uvm_object_utils(lpi_basic_seq) is a UVM factory registration macro used in
...
It tells UVM:

“This class is a UVM object”

“Register this class with the UVM factory”

“Allow this object to be created dynamically using the factory methods”
This macro automatically generates code internally for:

## 1. Factory registration

So UVM can recognize the class.
...

Polymorphism
Now wherever base_seq is created using factory, error_seq gets created automatically.

## Difference between object and component macros:

For sequence/items/transactions (objects):
`uvm_object_utils(my_item)

### Figures on this page:

![[UVM Theory_p1_img1.png]]

![[UVM Theory_p1_img2.png]]


## Page 2: 2. Connect Phase: Connect TLM ports/exports.
...
## `uvm_component_utils(my_driver)…………………… for components with hierarchy + phases.
...
## Because components have hierarchy and phases.


## Why phases are needed?

A UVM testbench has many components:

test

env
...

scoreboard
If everyone starts working randomly, chaos happens. So UVM divides simulation into
ordered steps called phases.

## Common UVM phases


## 1. Build Phase: Used to create components.
...
![[UVM Theory_p2_img1.png]]


## Page 3: synchronized execution. But Objects like:


## 3. Run Phase: Actual simulation activity happens here.
...
## 4. Check Phase: Final checking after simulation.


## 5. Report Phase: Print pass/fail summaries.


## Why only components have phases?

Because components are permanent hierarchical blocks of the testbench. These need
...

sequence
are temporary data objects. They are created/used/deleted dynamically. Think of UVM
components like workers in a factory. Everyone follows same schedule. Phases are:
1. Build factory
2. Connect machines
3. Start production
4. Inspect products
...
![[UVM Theory_p3_img1.png]]

![[UVM Theory_p3_img2.png]]
...
# Source: UVM_module5_HVM.pdf


## Page 1: 4.1  What Is UVM?

17
© Springer International Publishing AG 2018
A.B. Mehta, ASIC/SoC Functional Design Veri(cation,
DOI 10.1007/978-3-319-59418-7_4

# Chapter 4


# UVM (Universal Veri<cation Methodology)
...
Ah, isn’t this now the mother of all veriXcation methodologies? Let us start with
...
ment. It is a class library that makes it easy to write conXgurable and reusable code.
...
18
hierarchy, transaction library model (TLM), conXguration database, etc., which
...
the environment. When coupled with the phased, Wexible-build infrastructure in
...
easily encapsulating components into reusable components, called veriXcation
components, to maximize reuse and minimize the time and effort required to build
a veriXcation environment.
...
Open VeriXcation Methodology (OVM) from Mentor, Universal Reuse Methodology
(URM) from Cadence, and VeriXcation Methodology Manual (VMM) from
...
came from OVM, and thus UVM was born. The users felt conXdent in adopting this
universal methodology and not get tied to a speciXc vendor. All vendors now sup-
...
4  UVM (Universal VeriXcation Methodology)

### Figures on this page:

![[UVM_module5_HVM_p2_img1.png]]
*1, there were three competing methodologies in the industry.*


## Page 3: 4.2  Polymorphism
...
Figure 4.2 [ (Accelera, Universal VeriXcation Methodology (UVM) 1.2 User’s
...
ment contains an agent for a speciXc DUT interface. The environment may
...
21
Fig. 4.2 UVM hierarchy (Accelera, Universal VeriXcation Methodology (UVM) 1.2 User’s guide)
...
![[UVM_module5_HVM_p5_img1.png]]
*2 UVM hierarchy (Accelera, Universal VeriXcation Methodology (UVM) 1.2 User’s guide)*


## Page 6: 4.3.1  UVM Testbench
...
the UVM Test class and conXgures the connections between them. If you have
...
1. Stimulus Generator (sequencer) that creates transaction-level trafXc to feed to
the driver.
...
![[UVM_module5_HVM_p6_img1.png]]
*3, UVM transaction-level testbench instantiates the DUT and the*


## Page 7: 4.3.3  UVM Environment
...
instantiates the design under test (DUT) and the UVM Test class and conXgures the
connection between them. A test is a class that encapsulates test-speciXc instruc-
tions written by the test writer.
The UVM Test typically:
1. Instantiate the top-level environment.
2. ConXgure the environment (via factory overrides or the conXguration
database).
...
allows inheritance and reuse of tests. Typically, a base test class is deXned that
instantiates and conXgures the top-level environment and is then extended to deXne
scenario-speciXc conXgurations such as which sequences to run, coverage parame-
ters, etc. The test instantiates the top-level environment just like any other veriXca-
tion component.
Typically, there is one base UVM Test with the UVM environment instantiation
and other common items. Individual tests will extend this base test and conXgure the
environment differently or select different sequences to run.

# 4.3.3  UVM Environment

The UVM environment is a component that groups together other veriXcation com-
...
environments. The top-level UVM environment encapsulates all the veriXcation
components targeting the DUT.
The top-level environment is a container that deXnes the reusable component
topology within the UVM Tests. The top-level environment instantiates and conXg-
ures the reusable veriXcation IP and deXnes the default conXguration of that IP as
required by the application. Multiple tests can instantiate the top-level environment
class and determine the nature of trafXc to generate and send for the selected con-
Xguration. Additionally, the tests can override the default conXguration of the top-
level environment to better achieve their goals.
4.3 UVM Hierarchy

## Page 8: 4.3.4  UVM Agent

24
As noted above, the top-level UVM environment can instantiate other UVM
environments. Typically for each interface to the DUT, you will Xnd a separate envi-
...
The UVM agent is a hierarchical component that groups together other veriXcation
components that are dealing with a speciXc DUT interface. Agent includes a UVM
sequencer to manage stimulus Wow, a UVM driver to apply stimulus to the DUT
...
![[UVM_module5_HVM_p8_img1.png]]
*4.*


## Page 9: 4.3.5  UVM Sequence Item
...
it cannot control the IO of the DUT. You can dynamically conXgure an agent in
...
object in the UVM hierarchy. It is the deXnition of the basic transaction that will
then be used to develop UVM sequences. The sequence item deXnes the basic trans-
...
executed in a veriXcation model. They can include variables, constraints, and even
...
## Page 10: 4.3.6  UVM Sequence

26

# 4.3.6  UVM Sequence

After a basic uvm_sequence_item has been created, the veriXcation environment
...
the transaction (sequence item) are of type “rand,” if we want to test just a speciXc
...
Then we deXne the task body which is the gist of the sequence. In the task body(), we
Xrst deXne a seq_item of type lpi_seq_item which is what the task body will work on.
The “for” loop will iterate for a random number of times (since num_of_trans is
declared “rand”). It will Xrst start_item (start the sequence) which is a call that
...
ate an error and abort and will print the transaction information and do a (nish_item
...
The sequencer controls the Wow of request and response sequence items between
sequences and the driver. UVM sequencer is a simple component that serves as an
arbiter for controlling transaction Wow from multiple stimulus sequences.
...
item_port deXned respectively. The user needs to connect them using TLM connect
method.
Example:
...
item_port) are deXned for communication between sequencer and driver.
...
Figure 4.5 [ (Accelera, Universal VeriXcation Methodology (UVM) 1.2 User’s
guide)] shows the building blocks of UVM class library that you can use to quickly
build well constructed, reusable, conXgurable components and testbenches. The
library contains base classes, utilities, and macros.
The advantages of using the UVM class library [ (Accelera, Universal VeriXcation
Methodology (UVM) 1.2 User’s guide)] include:
(a) A robust set of built-in features—The UVM class library provides many fea-
tures that are required for veriXcation, including complete implementation of
...
## Page 14: 4.5.1  Basic Transaction-Level Communication

30
The UVM class library also provides various utilities to simplify the develop-
ment and use of veriXcation environments. These utilities support conXgurability by
...
tion by providing a standard communication infrastructure between veriXcation
components (TLM) and Wexible veriXcation environment construction (UVM fac-
...
“export.” A TLM “port” deXnes a set of methods (aka the Application Programming
...
Fig. 4.5 UVM class library hierarchy (Accelera, Universal VeriXcation Methodology (UVM) 1.2
User’s guide)
4  UVM (Universal VeriXcation Methodology)

### Figures on this page:

![[UVM_module5_HVM_p14_img1.png]]
*6 shows an example of a single producer connected to a single consumer.*
...
by the consumer. The semantics of the put operation are deXned by TLM. In this
...
the interfaces are well deXned.
...
Here’s a simple example (Accelera, Universal VeriXcation Methodology (UVM)
1.2 User’s guide).
...
![[UVM_module5_HVM_p16_img1.png]]
*7). In other words, here the consumer gets a*


## Page 17: 4.5.2  Hierarchical Connections
...
passing from the port to the export and back again. The direction of data Wow (from
...
called by the producer. UVM provides the uvm_tlm_Xfo channel to facilitate such
communication. The uvm_tlm_Xfo implements all the TLM interface methods, so
the producer puts the transaction into the uvm_tlm_Xfo, while the consumer inde-
...
![[UVM_module5_HVM_p17_img1.png]]
*8.*

![[UVM_module5_HVM_p17_img2.png]]
*8.*


## Page 18: 4.5.3  Analysis Ports and Exports
...
The producer contains “stim,” “Xfo,” and “conv,” while the consumer contains
“Xfo” and “drv”.
...
![[UVM_module5_HVM_p18_img1.png]]
*10). As we*


## Page 19: 4.6.1  Build Phases
...
analysis_Xfo, which is a uvm_tlm_Xfo that also includes an analysis export, to
allow blocking components access to the analysis transaction stream. The  analysis_
Xfo is unbounded, so the monitor’s write() call is guaranteed to succeed immedi-
ately. The analysis component may then get the transactions from the analysis_Xfo
whenever it pleases.
...
Xnally, the cleanup phase gathers details on the Xnal DUT state, processes the simu-
lation results, and does simulation results analysis and reporting.
The following provides at Xner granularity the phases at play in UVM.

# 4.6.1  Build Phases

The build phases are executed at the start of the UVM testbench simulation, and
their overall purpose is to construct, conXgure, and connect the testbench compo-
...
layer in the component hierarchy can be conXgured by the level above. During the
...
![[UVM_module5_HVM_p20_img1.png]]
*11 UVM phases (MentorGraphics)*


## Page 21: 4.6.2  Run-Time Phases

37
4.6.1.3  end_of_elaboration
The end_of_elaboration phase is used to make any Xnal adjustments to the struc-
ture, conXguration, or connectivity of the testbench before simulation starts. Its
...
easily migrated to the UVM. The other phases were added to UVM to give Xner
run-time phase granularity for tests, scoreboards, and other similar components. It
is expected that most testbenches will only use reset, conXgure, main, and shutdown
...
banners, testbench topology, or conXguration information. It is called in bottom-up
order.
4.6.2.2  Run
...
38
4.6.2.4  Reset
The reset phase is reserved for DUT or interface-speciXc reset behavior. For
...
4.6.2.6  pre_con<gure
The pre_conXgure phase is intended for anything that is required to prepare for the
DUT’s conXguration process after reset is completed, such as waiting for compo-
nents (e.g., drivers) required for conXguration to complete training and/or rate nego-
...
this phase.
4.6.2.7  Con<gure
The conXgure phase is used to program the DUT and any memories in the testbench
...
4.6.2.8  post_con<gure
The post_conXgure phase is used to wait for the effects of conXguration to propa-
...
## Page 23: 4.6.3  Cleanup Phases

39
4.6.2.10  Main
This is where the stimulus speciXed by the test case is generated and applied to the
...
4.6.2.11  post_main
This phase is used to take care of any Xnalization of the main phase. I do not antici-
...
Perform any Xnal activities before exiting the active simulation phases. At the end
...
results to Xle. This phase is usually used by analysis components.
4.6.3.4  Final
The Xnal phase is used to complete any other outstanding actions that the testbench
...
## Page 25: 4.7.1  Modeling a Sequence Item
...
library provides the uvm_sequence_item base class. Every user-deXned sequence
